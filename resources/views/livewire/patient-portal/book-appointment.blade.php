<div class="max-w-3xl mx-auto py-10 px-6">
    ...
    <div class="mb-12">
        <div class="flex items-center justify-center max-w-md mx-auto">
            <div class="flex flex-col items-center">
                <div class="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm transition-all duration-300 @if($step >= 1) bg-gradient-to-br from-teal-600 to-teal-700 text-white shadow-lg shadow-teal-600/20 @else bg-stone-200 text-stone-500 @endif" style="font-family:'Outfit',sans-serif">
                    @if($step > 1)<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>@else 1 @endif
                </div>
                <span class="text-xs mt-2 font-semibold @if($step >= 1) text-teal-600 @else text-stone-400 @endif" style="font-family:'Work Sans',sans-serif">Tratamiento</span>
            </div>
            <div class="flex-1 h-0.5 mx-3 rounded-full transition-all duration-300 @if($step > 1) bg-teal-600 @else bg-stone-200 @endif"></div>
            <div class="flex flex-col items-center">
                <div class="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm transition-all duration-300 @if($step >= 2) bg-gradient-to-br from-teal-600 to-teal-700 text-white shadow-lg shadow-teal-600/20 @else bg-stone-200 text-stone-500 @endif" style="font-family:'Outfit',sans-serif">
                    @if($step > 2)<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>@else 2 @endif
                </div>
    ...
    <div class="bg-white rounded-[2rem] border border-teal-500/10 shadow-lg overflow-hidden">
        <div class="px-6 py-8 sm:px-8" style="font-family:'Work Sans',sans-serif">
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <button type="button" wire:click="$set('selectedProcedureId', {{ $procedure->id }})"
                        class="relative rounded-2xl border-2 {{ $selectedProcedureId == $procedure->id ? 'border-teal-500 bg-teal-50/50 ring-2 ring-teal-200/50' : 'border-stone-200 bg-white hover:border-teal-300 hover:shadow-md' }} p-5 text-left transition-all duration-200">
                        @if($selectedProcedureId == $procedure->id)
                        <div class="absolute top-3 right-3"><svg class="w-5 h-5 text-teal-600" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg></div>
                        @endif
                        <div class="flex items-start gap-3">
                            <div class="flex-shrink-0 w-10 h-10 rounded-xl {{ $selectedProcedureId == $procedure->id ? 'bg-teal-100 text-teal-600' : 'bg-stone-100 text-stone-500' }} flex items-center justify-center transition-colors">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-stone-800 truncate">{{ $procedure->procedure_name }}</p>
                                <p class="text-sm font-bold text-teal-700 mt-1.5" style="font-family:'Outfit',sans-serif">${{ number_format($procedure->price, 0, ',', '.') }}</p>
                                @if($procedure->duration)<p class="text-xs text-stone-400 mt-1 flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>{{ $procedure->duration }} min</p>@endif
                            </div>
                        </div>
                    </button>
                    @endforeach
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="date" wire:model.live="selectedDate" id="date" min="{{ date('Y-m-d') }}"
                        class="block w-full rounded-xl border-stone-300 shadow-sm focus:border-teal-500 focus:ring-teal-500 py-3 px-4 text-stone-800">
                    @error('selectedDate') <p class="text-rose-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                @if($selectedDate)
                <div>
                    ...
                    <div class="grid grid-cols-3 sm:grid-cols-4 gap-3">
                        ...
                        <button type="button" wire:click="$set('selectedTimeSlot', '{{ $slot }}')"
                            class="{{ $selectedTimeSlot === $slot ? 'bg-gradient-to-r from-teal-600 to-teal-700 text-white shadow-lg shadow-teal-600/20 border-transparent' : 'bg-white text-stone-700 border-stone-200 hover:border-teal-300 hover:bg-teal-50/50' }} border-2 rounded-xl py-3 text-sm font-semibold transition-all duration-200">
                            {{ $slot }}
                        </button>
                        @endforeach
                    </div>
        ...
        <div class="bg-[#FFFBF5] px-6 py-5 sm:px-8 flex justify-between items-center border-t border-stone-100">
            @if($step > 1)
            <button wire:click="previousStep" type="button" class="inline-flex items-center py-2.5 px-5 border border-stone-300 shadow-sm text-sm font-semibold rounded-xl text-stone-700 bg-white hover:bg-stone-50 transition-colors" style="font-family:'Work Sans',sans-serif">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                Atras
            </button>
            @else
            <a href="{{ URL::signedRoute('portal.dashboard', ['patient' => $patient]) }}" class="inline-flex items-center py-2.5 px-5 border border-stone-300 shadow-sm text-sm font-semibold rounded-xl text-stone-700 bg-white hover:bg-stone-50 transition-colors" style="font-family:'Work Sans',sans-serif">
            ...
            @if($step < 3)
            <button wire:click="nextStep" type="button" class="inline-flex items-center py-2.5 px-6 border border-transparent shadow-sm text-sm font-semibold rounded-xl text-white bg-teal-600 hover:bg-teal-700 transition-all shadow-lg shadow-teal-600/20" style="font-family:'Work Sans',sans-serif">
                Siguiente
